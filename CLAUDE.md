...
```bash
cd ButtonPattern
make simulate       # Verilator simulation → outputs image.ppm + button tests
...
### Testbench Pattern
All projects share the same `testb.h` (in the repo root) — a reusable C++ template that wraps any Verilated module, handles clock toggling, and optionally writes VCD traces. Project-specific testbenches (e.g., `blinky_tb.cpp`) instantiate it. Makefiles pass `-CFLAGS "-I.."` so subdirectory testbenches can find it.

### HDMI Pipeline (TestPattern / ButtonPattern)
Both HDMI projects share sources from `hdmi_common/` — a single canonical location for all shared RTL:
1. `clock.v` — ECP5 PLL generates 25 MHz (pixel clock) and 250 MHz (TMDS bit clock) from the 25 MHz crystal.
2. `llhdmi.v` — VGA timing state machine (640×480 @ 60 Hz); emits `o_rd`, `o_newline`, `o_newframe` control signals; instantiates `tmds_serial`.
3. `tmds_serial.v` — Serializes three 10-bit TMDS words to 1-bit streams at 250 MHz (mod-10 counter + 3× shift registers).
4. `TMDS_encoder.v` — 8b/10b TMDS encoder (one instance per RGB channel); maintains DC balance via a running disparity counter.
5. `vgatestsrc.v` — Pixel source; generates the color-bar test pattern. `i_btn[5:0]` overrides the pattern with a solid color when any button is held. TestPattern wires `i_btn` to zero.
6. `OBUFDS.v` — Differential output buffer stub for simulation; replaced by ECP5 native cells during synthesis.
...
### Synthesis Scripts
Each project has a `.ys` Yosys script that reads Verilog sources, synthesizes for ECP5, and emits a JSON netlist. TestPattern and ButtonPattern use `ysgen.sh` to generate the `.ys` file dynamically; sources are passed as `../hdmi_common/<file>` paths.
